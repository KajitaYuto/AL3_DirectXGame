...
void GameScene::Initialize() {
	...
	for (int i = 0; i < 2; i++) {
		for (int x = 0; x < 10; x++) {
			worldTransform_[i][x].scale_ = {5.0f, 5.0f, 5.0f};
			worldTransform_[i][x].translation_ = {-40.0f+10.0f*x, 20.0f-40.0f*i, 0.0f};
			worldTransform_[i][x].Initialize();
		}
	}
...
void GameScene::Update() {
	...
	// viewProjection_.eye.z); debugText_->SetPos(50, 70); debugText_->Printf("target:(%f,%f,%f)",
	...
	// viewProjection_.up.z); debugText_->SetPos(50, 110);
	// debugText_->Printf("fovAngleY(Degree):%f", XMConvertToDegrees(viewProjection_.fovAngleY));
	// debugText_->SetPos(50, 130);
	// debugText_->Printf("nearZ:%f",viewProjection_.nearZ);
	// debugText_->SetPos(50, 150);
	// debugText_->Printf("Root:(%f,%f,%f)",
	// worldTransform_[PartId::Root].translation_.x,worldTransform_[PartId::Root].translation_.y,
...
#pragma region 3Dオブジェクト描画
	...
	for (int i = 0; i < 2; i++) {
		for (int x = 0; x < 10; x++) {
			model_->Draw(worldTransform_[i][x], viewProjection_, textureHandle_);
		}
	}
